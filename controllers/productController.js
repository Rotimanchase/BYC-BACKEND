...
export const addProduct = async (req, res) => {
    try {
        ...
        if (
            ...
            return res.status(400).json({ success: false, message: 'All required fields and at least one image are required' });
        }
        ...
        if (!req.files.every((file) => file.path)) {
            return res.status(400).json({ success: false, message: 'Invalid image files' });
        }
        const images = [...new Set(req.files.map((file) => path.join('/Uploads', file.filename)))];
...
export const updateProduct = async (req, res) => {
    try {
        ...
        if (req.files && req.files.length > 0) {
            ...
            updateData.productImage = imagesUrl;
        }
...
export const getProduct = async (req, res) => {
  try {
      ...
      console.log('Populated product reviews:', JSON.stringify(product.reviews, null, 2));
